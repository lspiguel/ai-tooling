...
1. **What did the client ask for, and what does the environment currently look like?** → `<client>-Context/`
2. **What is actually built?** → `<client>-d365/`
3. **What does everyone else need to know about it?** → `<client>-wiki/`

Collapsing them into one repository is the tempting shortcut and it fails in a predictable way: personal working notes end up in team pull requests, published documentation gets reviewed as if it were code, and the client's shared repo accumulates material that was never cleared to live there. Three repositories, laid out as siblings so a single agent session can read all of them.
...
|---|---|---|---|
| **What it is** | Personal context: exported packs, story folders, engagement documents | The project repository: unpacked solutions, source code, docs-as-code | Live documentation, published to the team and the client |
| **Audience** | You (and your assistant) | The delivery team | Everyone — including people who never open an IDE |
| **Ownership** | Personal, local | Team, shared, PR-reviewed | Team, shared; the wiki's own review flow |
| **Lifecycle** | Refreshed when the environment moves | Every commit of every story | Updated when behaviour or process changes |
| **Hosted** | Local (or a location the data agreement covers) | ADO Repos / GitHub | ADO wiki git repo (`<project>.wiki`) or equivalent |
| **The assistant** | Reads for intent and current environment state | Reads *and writes* — this is where it builds | Writes drafts; a human publishes |
| **Rows it feeds** | [1.3] [2.3] [5.3] | [1.3] → [6.3] | [6.3] |
...
- **`webresources/` flows in one direction only** — either authored here and deployed *into* the solution, or unpacked *out of* it. Mixing the two causes merge pain.
...
`docs/` in the project repo and the wiki are **not** duplicates: `docs/` holds artifacts that belong next to the code and are reviewed with it (ADRs, developer runbooks, architecture notes for the build); the wiki holds what the wider engagement reads. [6.3](./6.3-documentation.md) is where content is generated from the repo and pushed here.

---

## Laying them out on disk

Clone all three as **siblings under one parent folder** so a single agent session can read across them:
...
The payoff is a single prompt that spans all three: *"Read the story in `<client>-Context/1234-*`, check it against the unpacked solution in `<client>-d365/solutions/Core/src/`, and tell me which wiki pages under `Functional/` will need updating."* That question is unanswerable in any one repository alone.
...
| What does the environment's model look like? | `<client>-Context/context-exporter/output/` or `<client>-d365/context/` | File read |
...
| Why was it built that way? | `<client>-d365/docs/` (ADRs) | File read |
