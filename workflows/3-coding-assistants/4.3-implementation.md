...
**Agent Mode — automated implementation with a human in the loop.** This is the heavy lifting: plugins, PCF controls, web resources, Azure Functions, and pipeline YAML, built inside the grounded repo with build/test feedback loops. The assistant also covers snippet-level work (Power Fx, FetchXML, OData), verified against the unpacked solution and live schema instead of trusting a manual paste. Where skills and tooling are unavailable — work that must be done by hand in the maker portal — fall back to step-by-step configuration instructions.
...
> Append *"Verify every table and column logical name via Dataverse MCP `describe` before finalizing the query."*
