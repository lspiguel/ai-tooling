...
| **[1] Specification** | Draft requirements, user stories, ACs from stakeholder notes. The **AI Assistant** mines Teams/email/SharePoint for source material. Reconcile asks against the *existing* model (Context Exporter grounding) to catch "we already have a field for that." | Draft requirements, user stories, ACs from stakeholder notes. Coding assistant + repo + Context Exporter + **ADO MCP** + **Dataverse MCP** (`describe`, `search`) → grounded gap analysis and a technical spec that names *real* entities/attributes/plugins. Feasibility checked against the unpacked solution, not from memory. |
| **[2] Planning** | Decompose epics → features → stories, sequence, risk register, estimation rationale; produce the planning doc/deck. Push back into ADO using scripting. | Decompose epics → features → stories, sequence, risk register, estimation rationale; generate the **ADO work-item hierarchy via ADO MCP**; dependency + sequencing analysis from solution structure; draft ADRs (architecture skill). |
| **[3] Tasking** | Turn features into task checklists with crisp descriptions and a clear Definition of Done. | Create ADO tasks via MCP; scaffold branches; emit task-level technical notes grounded in the repo. |
| **[4] Implementation** | The **AI Assistant** helps in generating/explaining Power Fx, JS web resources, FetchXML, OData. Rubber-duck design. Use **maker-portal copilots** (Copilot in Power Apps / Power Automate, generative pages) for low-code. Pro-Code direct implementation is out of current capabilities for **AI Assistants**. | The core build: plugins, PCF, web resources, Azure Functions, pipeline YAML. The **Coding Assistant** also helps in generating/explaining Power Fx, JS web resources, FetchXML, OData.. pac CLI / **Dataverse coding-agent plugin** for environment gestures. |
| **[5] Validation / peer review** | Review design docs; generate test scenarios + UAT scripts; sanity-check an approach; cross-check work against the requirements doc (AI Assistant). | Code review via a **`d365-code-reviewer.agent.md`** persona enforcing StyleCop + naming; PR descriptions + self-review; run tests; **Plugin A — Component Snapshot & Drift Tracker** for config drift; security/perf review; ADO/GitHub PR Copilot. |
| **[6] Documentation / information sharing** | User docs, runbooks, release notes, training material; publish to ADO wikis; **re-run Context Exporter** so the schema packs stay current as living docs. | Generate technical docs + ADRs from code; publish to ADO wikis; keep `copilot-instructions.md` / `AGENTS.md` honest (`/init` refresh). |
