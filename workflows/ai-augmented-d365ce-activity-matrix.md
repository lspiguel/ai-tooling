...
- **Rows** are the activities a D365 CE practitioner moves through: one grounding layer, two one-time setups, then the iterative loop of spec → plan → task → implement → validate → document.
...
|---|---|---|
| **[A] Grounding** <br><sub>backbone</sub> | Extracts from live environments into markdown or text files, upload to the AI assistant. | Solutions exported and unpacked, git repositories for all source code and documentation. |
| **[B] Initial one-time setup** <br><sub>per person / per team</sub> | Stand up a reusable **"D365 CE house style" Project / Custom GPT** seeded with conventions (publisher prefix rules, StyleCop summary, user-story + AC templates, the tool taxonomy). Enable an AI Assistant. Curate a personal prompt library. [[B.1]](./1-general-purpose-assistants/b.1-initial-setup.md) | Baseline the dev box: VS Code + Power Platform Tools + Copilot/Cursor, **pac CLI**, XrmToolBox + plugins (**D365 Context Exporter**, FetchXML Builder). Build a **template repo** carrying canonical instructions and skills (i.e. `.github/copilot-instructions.md`, `AGENTS.md`, `.claude/*`, etc.). Register Dataverse MCP + ADO/GitHub MCP. Fix the solution-unpack git layout. [[B.3]](./3-coding-assistants/b.3-initial-setup.md) |
| **[C] Project setup** <br><sub>per client / per engagement</sub> | Per-client **grounded Project** seeded with Context Exporter `.context.md` packs + SoW + architecture; pin client data-handling rules. Use an **AI Assistant scoped to the client tenant** for anything touching live data. [[C.1]](./1-general-purpose-assistants/c.1-project-setup.md) | Setup ALM solution if available, if not setup `pac solution export`/`pac solution unpack` script to automate solution registration into git; clone repos; drop Context Exporter packs into `/context/`; fork the template instruction/agent files and fill client specifics (prefix, env URLs, naming); enable this environment's **Dataverse MCP** + ADO board MCP; run or clone initializations to bootstrap instructions from the actual repo. [[C.3]](./3-coding-assistants/c.3-project-setup.md) |
| **[1] Specification** | Draft requirements, user stories, ACs from stakeholder notes. The **AI Assistant** mines Teams/email/SharePoint for source material. Reconcile asks against the *existing* model (Context Exporter grounding) to catch "we already have a field for that." [[1.1]](./1-general-purpose-assistants/1.1-specification.md) | Draft requirements, user stories, ACs from stakeholder notes. Coding assistant + repo + Context Exporter + **ADO MCP** + **Dataverse MCP** (`describe`, `search`) → grounded gap analysis and a technical spec that names *real* entities/attributes/plugins. Feasibility checked against the unpacked solution, not from memory. [[1.3]](./3-coding-assistants/1.3-specification.md) |
| **[2] Planning** | Decompose epics → features → stories, sequence, risk register, estimation rationale; produce the planning doc/deck. Push back into ADO using scripting. [[2.1]](./1-general-purpose-assistants/2.1-planning.md) | Decompose epics → features → stories, sequence, risk register, estimation rationale; generate the **ADO work-item hierarchy via ADO MCP**; dependency + sequencing analysis from solution structure; draft ADRs (architecture skill). [[2.3]](./3-coding-assistants/2.3-planning.md) |
| **[3] Tasking** | Turn features into task checklists with crisp descriptions and a clear Definition of Done. [[3.1]](./1-general-purpose-assistants/3.1-tasking.md) | Create ADO tasks via MCP; scaffold branches; emit task-level technical notes grounded in the repo. [[3.3]](./3-coding-assistants/3.3-tasking.md) |
| **[4] Implementation** | The **AI Assistant** helps in generating/explaining Power Fx, JS web resources, FetchXML, OData. Rubber-duck design. Use **maker-portal copilots** (Copilot in Power Apps / Power Automate, generative pages) for low-code. Pro-Code direct implementation is out of current capabilities for **AI Assistants**. [[4.1]](./1-general-purpose-assistants/4.1-implementation.md) | The core build: plugins, PCF, web resources, Azure Functions, pipeline YAML. The **Coding Assistant** also helps in generating/explaining Power Fx, JS web resources, FetchXML, OData.. pac CLI / **Dataverse coding-agent plugin** for environment gestures. [[4.3]](./3-coding-assistants/4.3-implementation.md) |
| **[5] Validation / peer review** | Review design docs; generate test scenarios + UAT scripts; sanity-check an approach; cross-check work against the requirements doc (AI Assistant). [[5.1]](./1-general-purpose-assistants/5.1-validation-peer-review.md) | Code review via a **`d365-code-reviewer.agent.md`** persona enforcing StyleCop + naming; PR descriptions + self-review; run tests; **Plugin A — Component Snapshot & Drift Tracker** for config drift; security/perf review; ADO/GitHub PR Copilot. [[5.3]](./3-coding-assistants/5.3-validation-peer-review.md) |
| **[6] Documentation / information sharing** | User docs, runbooks, release notes, training material; publish to ADO wikis; **re-run Context Exporter** so the schema packs stay current as living docs. [[6.1]](./1-general-purpose-assistants/6.1-documentation.md) | Generate technical docs + ADRs from code; publish to ADO wikis; keep `copilot-instructions.md` / `AGENTS.md` honest (`/init` refresh). [[6.3]](./3-coding-assistants/6.3-documentation.md) |
...
### A.1. D365 Context Exporter — the grounding layer for Column 1
...
### A.3. Solution export and unpacking into a git repository also containing source code, Documentation as Code — grounding layer for Column 3
...
### B.3. The instruction / agent / skill file stack — the grounding layer for Column 3

Per the "context injection over custom agents" principle, this is now well-supported natively and stays file-based (no bespoke agent system to maintain). Conventions vary by assistant; the GitHub Copilot set is shown below, with the Claude Code equivalent alongside:

- `.github/copilot-instructions.md` (Claude Code: `CLAUDE.md`) — always-on, project-wide. Keep it under ~1,000 words or the signal gets buried. Publisher prefix, no early binding, StyleCop, Repository pattern, "tables change fast → describe via MCP, don't assume."
- `AGENTS.md` — workflow rules: how to build, which test command, what "done" means. Nestable per subdirectory for a monorepo (plugins vs PCF vs web resources). Read natively by both ecosystems.
- `.github/agents/*.agent.md` (Claude Code: `.claude/agents/*`) — lightweight named personas (plugin-dev, code-reviewer, PCF) with a constrained tool list and an optional pinned model. These *are* "context injection," not a custom agent platform — exactly the line you want.
- `.github/skills/<name>/SKILL.md` (Claude Code: `.claude/skills/<name>/SKILL.md`) — reusable task instructions auto-discovered by the agent.

Ship these from the template repo so every engagement starts consistent.

### C.3. Dataverse MCP server
...
|---|---|---|
| **House starter pack** (`copilot-instructions.md` + `AGENTS.md` + `.agent.md` personas) | [B] Initial setup, all of col 3 | Column-3 analogue of the exporter's reference config. Distributable, versioned. |
...
| **`d365-gap-analysis` skill** | [1] Specification | Chains a Context Exporter pack + ADO MCP + Dataverse `describe` into a structured "new vs existing" report. |
...
| **Plugin B — Import Dependency Pre-Flight** | [2] Planning, [5] Validation | Already specced (read-only, order-sensitive). Sequencing input. |
| **Plugin 9 — PCF Scaffold Orchestrator** | [3] Tasking, [4] Implementation | Already specced. PAC CLI orchestration with field/dataset split. |
