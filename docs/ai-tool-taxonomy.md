...
The taxonomy classifies where an AI runs; [docs/agents](./agents/) documents what specific agents can actually do. It holds per-agent infrastructure and capability profiles (Claude, Gemini, GitHub Copilot, Grok) produced with a reusable [capability interrogation prompt sequence](./agents/agent-capability-interrogation-prompts.md) — an empirical companion to this classification.
